...
func (client Client) TpsCurrentIP(signType SignType) (string, error) {
	ep := TpsCurrentIP
...
func (client Client) ChangeTpsIP(signType SignType) (string, error) {
	ep := ChangeTpsIP
	params := client.getParams(ep, signType, nil)
	res, err := client.getBaseRes("GET", ep, params)
	if err != nil {
...
func (client Client) GetTpsIp(num int, signType SignType, kwargs map[string]interface{}) ([]string, error) {
	ep := GetTpsIp
	if kwargs != nil {
		kwargs["num"] = num
	} else {
