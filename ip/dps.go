...
// GetDps 获取私密代理
// return: 代理slice
func (client Client) GetDps(num int, signType SignType, kwargs map[string]interface{}) ([]string, error) {
	ep := GetDpsProxy
	if kwargs != nil {
		kwargs["num"] = num
	} else {
...
func (client Client) CheckDpsValid(proxyList []string, signType SignType) (map[string]bool, error) {
	if len(proxyList) == 0 {
...
// return: map[string]bool, key: proxy, value:true/false
func (client Client) GetDpsValidTime(proxyList []string, signType SignType) (map[string]string, error) {
	if len(proxyList) == 0 {
