...
// return: 过期时间字符串
func (client Client) GetOrderExpireTime(signType SignType) (string, error) {
	ep := GetOrderExpireTime
...
// return: 鉴权信息字典
func (client Client) GetProxyAuthorization(plaintext int, signType SignType) (map[string]string, error) {
	ret := make(map[string]string)
	ep := GetProxyAuthorization
...
// return: ip白名单slice
func (client Client) GetIPWhitelist(signType SignType) ([]string, error) {
	ep := GetIPWhitelist
	params := client.getParams(ep, signType, nil)
	res, err := client.getBaseRes("GET", ep, params)
	if err != nil {
...
// return: true/false
func (client Client) SetIPWhitelist(ipList []string, signType SignType) (bool, error) {
...
// return: ip余额
func (client Client) GetIPBalance(signType SignType) (int, error) {
	ep := GetIPBalance
	params := client.getParams(ep, signType, nil)
	res, err := client.getBaseRes("GET", ep, params)
	if err != nil {
...
//GetUA 获取User Agent
// return: user agent数组
func (client Client) GetUA(num int, signType SignType) ([]string, error) {
	ep := GetUA
		...
		if count, ok := data["count"].(float64); ok && count == 0 {
			...
			if ua_list, ok := data["ua_list"].([]interface{}); ok {
				...
				for _, v := range ua_list {
					uaList = append(uaList, TypeSwitcher(v))
				}
				return uaList, nil
			}
			return []string{}, nil
...
// return: 地区编码信息字典
func (client Client) GetAreaCode(area string, signType SignType) (map[string]string, error) {
	ret := make(map[string]string)
	ep := GetAreaCode
	kwargs := make(map[string]interface{})
	kwargs["area"] = area
...
// GetAreaCode 获取账户余额
// return: 账户余额字符串
func (client Client) GetAccountBalance(signType SignType) (string, error) {
	ep := GetAccountBalance
	params := client.getParams(ep, signType, nil)
	res, err := client.getBaseRes("GET", ep, params)
	if err != nil {
		return "", err
...
func (client Client) CreateOrder(product string, pay_type string) (jsonRes, error) {
	...
	params := client.getParams(ep, HmacSha1, kwargs)
	res, err := client.getBaseRes("GET", ep, params)
	return res, err
}
